        ...
        switch (data.getType()) {
            case SCHEMA:
                ...
                dataExecutor.execute(null, structConvertor.convert(schemaStruct), () -> {
                    schemaStatusService.update(schemaStruct.getSchemaName(), MigrateStageService.LocalStorageStatus.COMPLETE.getStatus());
                    ack(data);
            ...
            case TABLE:
                ...
                dataExecutor.execute(tableStruct.getSchemaName(), structConvertor.convert(tableStruct), () -> {
                    fullTableStatusService.update(tableStruct, MigrateStageService.LocalStorageStatus.COMPLETE.getStatus());
                    fullTableDataPositionService.create(new FullTableDataPositionORM(tableStruct.getSchemaName(), tableStruct.getTableName(), ""));
                    // 表结构迁移完成，开始迁移数据
            ...
            case FULL_DATA:
                ...
                dataExecutor.executeBatch(tableData.getSchemaName(), structConvertor.convert(tableData), tableData.getContain(), () -> {
                    fullTableDataPositionService.updatePosition(tableData);
                    ack(tableData);
                    LOG.info("## Complete Drainer Data {}", data);
                });
                break;
